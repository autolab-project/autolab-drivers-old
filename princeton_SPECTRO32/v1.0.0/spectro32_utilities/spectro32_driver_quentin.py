...
class Driver_LOCAL(Driver_DLL):
    """This Class exclusively inherits from Driver_DLL class"""
    
    # in server.py: self.spectro = spectr.Driver_LOCAL() 
    
    Driver_DLL.__init__(masterfile=r'C:\Program Files\Princeton Instruments\PVCAM\SDK\master.h',
                     pvcamfile=r'C:\Program Files\Princeton Instruments\PVCAM\SDK\pvcam.h',**kwargs)
    
    def connect():
        # send the instance itself or a dict of all attributes
        
    def execute()
        try:
            exec(self.spectro+f'{command}')
...
class Driver_REMOTE(Driver):
    """This Class exclusively inherits from Driver_DLL class"""
    
    def __init__(self,address='192.168.0.2',camera='Camera1',masterfile=r'C:\Program Files\Princeton Instruments\PVCAM\SDK\master.h',pvcamfile=r'C:\Program Files\Princeton Instruments\PVCAM\SDK\pvcam.h',**kwargs):
        import socket 
    ...
    def connection():
        # recv an instance or the list of all attributes
        # then set all the attributes as your self.attribute = self.query(attribute)
